...
test('value', (t) => {
  ...
  t.deepEqual(spec, new Map().set('Authorization', new Set([{ value: 'Bearer abc123' }])))
})

test('comment', (t) => {
  const spec = makeSpec()
  header({ name: 'Authorization', comment: 'Test authentication' }, spec)
  t.deepEqual(spec, new Map().set('Authorization', new Set([{ comment: 'Test authentication' }])))
})

test('value with charset', (t) => {
  const spec = makeSpec()
  header({ name: 'Content-Type', value: 'text/plaincharset=UTF-8' }, spec)
  t.deepEqual(spec, new Map().set('Content-Type', new Set([{ value: 'text/plaincharset=UTF-8' }])))
})
